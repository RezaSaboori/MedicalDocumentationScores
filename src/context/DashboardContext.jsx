...
export const DashboardProvider = ({ children }) => {
  ...
  // 1. Fetch available snapshots on mount
  useEffect(() => {
    fetchSnapshots()
      .then((snaps) => {
        setSnapshots(snaps);
        // Default to the latest period if none is selected
        if (snaps.length > 0 && !selectedPeriod) {
  ...
  // 2. Fetch dashboard data whenever the selected period changes
  useEffect(() => {
  ...
  // 3. Filter, enrich, and format data for the UI components
  const { data, availableYears } = useMemo(() => {
    ...
    // Reconstruct metrics that weren't explicitly saved in the DB schema
    const enrichRow = (row) => {
      const N = (row.E || 0) + (row.G || 0) + (row.A || 0) + (row.W || 0) + (row.F || 0) + (row.Z || 0);
      const N_safe = N || 1;
      return {
        ...row,
        rho_Z: row.Z / N_safe,
    ...
    const applyFilters = (rows) => {
      return rows.filter(row => {
        if (filters.selectedYear !== 'all' && row.year && String(row.year) !== String(filters.selectedYear)) {
          return false;
        }
    ...
    const enrichedCurrent = filteredCurrent.map(c => {
      ...
      const c_enriched = enrichRow(c);
      return {
        ...c_enriched,
        comparison: prev ? {
          PDI: prev.PDI,
          delta_PDI: c_enriched.PDI - prev.PDI,
          V: prev.V,
          delta_V: c_enriched.V - prev.V
        } : null
      };
    });
